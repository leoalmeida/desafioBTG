...
export class OrderCard {
  ...
  onAddOrderItem(): void {
    const dialogRef = this.dialogAcao.open(OrderItemDetails, {
      ...
      data: {
        id:
          this.order().itemList.length > 0
            ? Math.max(...this.order().itemList.map((i) => i.id)) + 1
            : 1,
    ...
    dialogRef.afterClosed().subscribe((result) => {
      if (result) {
        ...
        this.order().totalPrice = this.order().itemList.reduce(
  ...
  onUpdateOrderItem(orderItem: OrderItemType): void {
    ...
    dialogRef.afterClosed().subscribe((result) => {
      if (result) {
        const changed = this.order().itemList.findIndex(
          (i) => i.id === result.id,
        );
        if (changed !== -1) {
  ...
  onRemoveOrderItem(orderItem: OrderItemType): void {
    const removed = this.order().itemList.findIndex(
      (i) => i.id === orderItem.id,
    );
    if (removed !== -1) {
